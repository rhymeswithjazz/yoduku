...
- [ ] **Step 4: Run to verify pass** — `npm test` → pass 12, fail 0.
...
- [ ] **Step 4: Run to verify pass** — `npm test` → pass 17, fail 0.
...
- [ ] **Step 4: Run to verify pass** — `npm test` → pass 19, fail 0. Note the suite's wall time; if it exceeds ~60s the 7×7 clue selection is the culprit — raise the `floor` in `selectClues` (e.g. `ceil(total/3)`) and re-run.
...
- [ ] **Step 4: Run to verify pass** — `npm test` → pass 22, fail 0.
...
- [ ] **Step 4: Run to verify pass** — `npm test` → pass 27, fail 0.
...
- [ ] **Step 4: Run to verify pass** — `npm test` → pass 30, fail 0.
...
- [ ] **Step 4: Run to verify pass** — `npm test` → pass 35, fail 0.
...
- [ ] **Step 3: Run unit suite to confirm nothing regressed** — `npm test` → pass 35, fail 0.
